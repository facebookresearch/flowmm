...
class DockCSPNet(CSPNet):
    def forward(
        ...
        osda_dummy_edge_feats = dummy_edge_ids.repeat(osda_edges.shape[1], 1)

        osda_edges = torch.cat([osda_edges, batch.osda.edge_index], dim=1)
        osda_edge_feats = torch.cat(
            [osda_dummy_edge_feats, batch.osda.edge_feats], dim=0
        )
        osda_edge_feats = self.edge_embedding(osda_edge_feats)

        distance_vectors = -FlatTorus01.logmap(
            batch.osda.frac_coords[batch.osda.edge_index[0]],
            batch.osda.frac_coords[batch.osda.edge_index[1]],
        )
